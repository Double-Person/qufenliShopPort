		...
		<view class="goodsManage-content">
			<view class="goodsManage-content-left">
				<scroll-view scroll-y="true" :style="{ 'height':scrollHeight }">
					<view v-for="(item,index) in list" :key="index" :class="{'leftActive':index==leftIndex}" :data-index="index"
					 @tap="leftTap">
						{{item.NAME}}
					</view>
				</scroll-view>
...
<script>
	...
	import { getShopMsg, goodDelete } from "@/common/apis.js"
	export default {
		name: 'GoodsManager',
		components: {
			commonHeader
		...
		methods: {
			...
			// 获取商品列表
			async getGoods () {
				const { returnMsg, msgType } = await getShopMsg({ shop_id: uni.getStorageSync('shopId') })
			...
			// 删除商品
			delGoods(id){
			 goodDelete({goods_id:id}).then(res => {
				 console.log(res)
			 }) 
			}
		},
