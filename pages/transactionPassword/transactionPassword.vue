<template>
	<view class="loginPassword">
		...
		<input type="password" placeholder-style="color:#333" v-model="form.paypass" placeholder="请输入原密码"/>
...
<script>
	...
	import { changeDealPwd } from '@/common/apis.js'
	...
	export default {
		...
		methods:{
			async submit (){
				if(this.form.npaypass == '' ||  this.form.newpaypass== ''||  this.form.paypass== '') {
					uni.showToast({
						title: '请输入密码！',
						icon: 'none'
					})
